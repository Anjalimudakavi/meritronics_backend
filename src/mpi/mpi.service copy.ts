...
// // Step 4: Only update checklist items if their section already exists
// if (data.checklists?.length) {
//   for (const userChecklist of data.checklists) {
//     if (!userChecklist.name) continue;
...
//     if (!existingChecklist) continue; // ❌ Skip if checklist section doesn't exist

//     for (const userItem of userChecklist.checklistItems ?? []) {
//       if (!userItem.description) continue;

//       const existingItem = await this.prisma.checklistItem.findFirst({
//         where: {
//           sectionId: existingChecklist.id,
//           description: userItem.description,
//         },
//       });

//       if (!existingItem) continue; // ❌ Skip if item doesn't already exist
...
async update(id: string, data: UpdateMpiDto) {
  ...
  const updatedMpi = await this.prisma.mPI.update({
    ...
    data: {
      jobId: data.jobId,
      assemblyId: data.assemblyId,
        customer: data.customer, // ✅ Add this line
    },
  });

  // // Step 2: Update existing OrderForm (no create)
  // if (data.orderForms) {
  //   const existingOrderForm = await this.prisma.orderForm.findFirst({
  //     where: { mpiId: id },
  //   });
  ...
  if (existingOrderForm?.id) {
    await this.prisma.orderForm.update({
      where: { id: existingOrderForm.id },
      data: {
        ...data.orderForms, // Only update provided fields
      },
    });
  }
}

  // Step 3: Update Stations and their SpecificationValues
  if (data.stations?.length) {
    for (const station of data.stations) {
      if (!station.id) throw new Error('Station ID is required');
      ...
      // Update StationSpecification values
      if (station.specificationValues?.length) {
        for (const spec of station.specificationValues) {
  ...
  // Step 4: Update existing Checklists and their items
if (data.checklists?.length) {
  for (const checklist of data.checklists) {
    if (!checklist.id) continue;

    // Update checklist itself
